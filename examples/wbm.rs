...
pub fn main() -> anyhow::Result<()> {
    ...
    let block_cache =
        rocksdb::Cache::new_lru_cache((BLOCK_CACHE_SIZE_MIB + WRITE_BUFFER_SIZE_MIB) * 1 << 20);

    let write_buffer_manager = Arc::new(
        rocksdb::WriteBufferManager::new_write_buffer_manager_with_cache(
            BLOCK_CACHE_SIZE_MIB * 1 << 20,
            true,
            block_cache.clone(),
        ),
    );
    ...
    println!(
        "Buffer size: {}",
        ByteSize::b(write_buffer_manager.get_buffer_size() as u64).display()
    );
